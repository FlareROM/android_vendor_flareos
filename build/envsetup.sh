...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/flareos/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
function repopick() {
    T=$(gettop)
    $T/vendor/flareos/build/tools/repopick.py $@
}
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/flareos/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
